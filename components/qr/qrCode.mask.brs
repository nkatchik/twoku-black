...
function _qrCode_mask_getPenaltyScore() as integer
    ...
    for y = 0 to m.size - 1
        ...
        next
        result += m.finderPenaltyTerminateAndCount(runColor <> 0, runX, runHistory) * m.PENALTY_N3
    next
    ...
    for x = 0 to m.size - 1
        ...
        next
        result += m.finderPenaltyTerminateAndCount(runColor <> 0, runY, runHistory) * m.PENALTY_N3
    next
